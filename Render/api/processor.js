...
export async function processArticle(data) {
  ...
  try {
    ...
    // STEP 4: Update article using supabase utils
...
async function generateExplanationView(articleId, profile, baseArticle) {
  try {
    // Check if explanation already exists using supabase utils
    ...
    // Insert explanation view using supabase utils
    await supabase
      .from("explanation_views")
      .insert({
...
export async function processQuestion(data) {
  ...
  try {
    // Get profile using supabase utils
